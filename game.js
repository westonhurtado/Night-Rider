...
function init() {
    ...
    camera.position.set(0, 1.7, -0.5); // Position camera at rider's head height
    camera.rotation.x = -0.1; // Slight downward angle
    ...
    initEnhancedGraphics();

    const volumetricFog = addVolumetricFog();
    const dynamicEnv = addDynamicEnvironment();
    const lightTrails = createLightTrails();
    const interactiveParticles = addInteractiveParticles();

    initVegetation();
    initEnvironment();

    const fpElements = createFirstPersonElements();

    objectPool.initialize();
    
    // Add LOD to existing objects
    const objects = scene.children.filter(child => 
        child.userData.type === 'tree' || 
        child.userData.type === 'house'
    );
    
    objects.forEach(object => {
        lodManager.updateObject(object, camera.position);
    });
}
    ...
    if (gameActive && !tutorial) {
        ...
        obstacles.forEach((obstacle, index) => {
            ...
            if (Math.abs(obstacle.position.z - bike.position.z) < 1 &&
                Math.abs(obstacle.position.x - bike.position.x) < 0.8 &&
                !isJumping && !isInvincible) {
        ...
        powerUps.forEach((powerUp, index) => {
            ...
            if (Math.abs(powerUp.position.z - bike.position.z) < 1 &&
                Math.abs(powerUp.position.x - bike.position.x) < 0.8) {
        ...
        coins.forEach((coin, index) => {
            ...
            if (Math.abs(coin.position.z - bike.position.z) < 1 &&
                Math.abs(coin.position.x - bike.position.x) < 0.8) {
                updateCombo();
        ...
        if (Math.random() < 0.03) {
            const lane = Math.floor(Math.random() * 3);
            createCoin(lanePositions[lane], -50);
        }
        ...
        if (speedBoostActive || bike.position.y > 0.5) {
            particleSystem.createSpeedTrail(bike.position);
        }
        ...
        // Random color cycling during speed boost
        if (speedBoostActive && Math.random() < 0.1) {
            cycleBikeColor();
        }
        
        // Rotate wheels based on speed
        bike.children[0].children[3].rotation.x += gameSpeed * 0.5; // Front wheel
        bike.children[0].children[4].rotation.x += gameSpeed * 0.5; // Back wheel
        ...
        shaderEffects.roadGrid.uniforms.time.value += 0.01;
        shaderEffects.hologram.uniforms.time.value += 0.016;
        volumetricFog.material.uniforms.time.value += 0.005;
        
        // Update dynamic environment mapping
        dynamicEnv.update();
        
        // Update interactive particles
        updateInteractiveParticles(interactiveParticles, bike.position);
        
        // Update light trails
        updateLightTrails(lightTrails, bike.position);
        ...
        // Update first-person elements
        const turnAmount = (lanePositions[currentLane] - bike.position.x) * 0.1;
        
        // Animate arms and handlebars during turns
        fpElements.handlebarGroup.rotation.z = -turnAmount * 0.3;
        fpElements.leftArm.rotation.z = -turnAmount * 0.2;
        fpElements.rightArm.rotation.z = -turnAmount * 0.2;
        
        // Animate suspension based on speed and jumps
        const suspensionCompress = isJumping ? 
            Math.abs(Math.sin(Date.now() * 0.01)) * 0.1 : 
            Math.sin(Date.now() * 0.01) * 0.02 * gameSpeed;
        fpElements.frontWheel.position.y = -0.8 + suspensionCompress;
        
        // Update speedometer
        fpElements.updateSpeedometer(gameSpeed * 5);
        
        // Add hand movement during jump
        if (isJumping) {
            const jumpProgress = Math.sin(Date.now() * 0.005) * 0.1;
            fpElements.leftArm.position.y = -0.2 + jumpProgress;
            fpElements.rightArm.position.y = -0.2 + jumpProgress;
            fpElements.handlebarGroup.position.y = -0.2 + jumpProgress;
        }

        // Rotate front wheel
        fpElements.frontWheel.children[0].rotation.z += gameSpeed * 2;
    }
